...
export default function koa2FallbackApiMiddleware(options) {
	options = options || {} 
	...
	return function(ctx, next) {
      ...
      console.log(parsedUrl.pathname,rewrite.from)

      console.log(match)
			if (match !== null) {
				rewriteTarget = evaluateRewriteRule(parsedUrl, match, rewrite.to)
        logger('Rewriting', ctx.method, ctx.url, 'to', rewriteTarget);
        console.log('Rewriting', ctx.method, ctx.url, 'to', rewriteTarget);
